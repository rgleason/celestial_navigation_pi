...
double astrolabe::calendar::cal_to_jd(int yr, int mo, double day,
                                      bool gregorian) {
...
int astrolabe::calendar::cal_to_day_of_year(int yr, int mo, int dy,
                                            bool gregorian) {
...
void astrolabe::calendar::day_of_year_to_cal(int yr, int N, bool gregorian,
                                             int& mo, int& dy) {
  /*Convert a day of year number to a month and day in the Julian or Gregorian
  calendars.
...
void astrolabe::calendar::easter(int yr, bool gregorian, int& mo, int& dy) {
  /* Return the date of Western ecclesiastical Easter for a year in the Julian
...
bool astrolabe::calendar::is_dst(double jd) {
  ...
  If astrolabe.globals.daylight_timezone_name is None, the function always
  returns false.
  ...
  */
  if (astrolabe::globals::daylight_timezone_name.empty()) return false;
...
void astrolabe::calendar::jd_to_cal(double jd, bool gregorian, int& yr, int& mo,
                                    double& day) {
...
string astrolabe::calendar::lt_to_str(double jd, const string& zone,
                                      const string& level) {
  ...
  */
  ...
  char cstr[50];
  if (level == "second") {
    sprintf(cstr, "%d-%s-%02d %02d:%02d:%02d %s", yr, month.c_str(), iday, hr,
            mn, sec, zone.c_str());
    return cstr;
  }
  if (level == "minute") {
    sprintf(cstr, "%d-%s-%02d %02d:%02d %s", yr, month.c_str(), iday, hr, mn,
            zone.c_str());
    return cstr;
  }
  if (level == "hour") {
    sprintf(cstr, "%d-%s-%02d %02d %s", yr, month.c_str(), iday, hr,
            zone.c_str());
    return cstr;
  }
  if (level == "day") {
...
double astrolabe::calendar::sidereal_time_greenwich(double jd) {
  ...
  */
  ...
  const double theta0 = 280.46061837 + 360.98564736629 * (jd - 2451545.0) +
