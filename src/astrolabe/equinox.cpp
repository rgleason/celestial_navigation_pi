...
class Key {
public:
  Key(Epoch epoch, Season season) : epoch(epoch), season(season){};
...
const Data _tbl[] = {
    ...
    {pre1000, kSummer, 1721233.25401, 365241.72562, -0.05323, 0.00907,
     -0.00025},
    {pre1000, kAutumn, 1721325.70455, 365242.49558, -0.11677, -0.00297,
     0.00074},
    {pre1000, kWinter, 1721414.39987, 365242.88257, -0.00769, -0.00933,
     -0.00006},
    //
    // Meeus-1998 Table 27.B
    //
    {post1000, kSpring, 2451623.80984, 365242.37404, 0.05169, -0.00411,
     -0.00057},
    {post1000, kSummer, 2451716.56767, 365241.62603, 0.00325, 0.00888,
     -0.00030},
    {post1000, kAutumn, 2451810.21715, 365242.01767, -0.11575, 0.00337,
     0.00078},
    {post1000, kWinter, 2451900.05952, 365242.74049, -0.06223, -0.00823,
     0.00032}};
};
...
public:
  Table(const Data* begin, const Data* end) {
    for (const Data* p = begin; p != end; p++)
      table[Key(p->epoch, p->season)] =
          Value(p->d0, p->d1, p->d2, p->d3, p->d4);
  };
...
namespace {
const Term _terms[] = {{485, d_to_r(324.96), d_to_r(1934.136)},
...
double astrolabe::equinox::equinox_approx(int yr, Season season) {
  ...
  */
  if (yr < -1000 || yr > 3000)
    throw Error("astrolabe::equinox::equinox_approx: year is out of range = " +
                int_to_string(yr));
  if (season < kSpring || season > kWinter)
    throw Error("astrolabe::equinox::equinox_approx: unknown season = " +
  ...
  double S = 0.0;
  for (std::vector<Term>::const_iterator p = terms.begin(); p != terms.end();
       ++p)
    S += p->A * cos(p->B + p->C * T);
...
public:
  SeasonToCircle() {
    ...
    const Data tbl[] = {Data(kSpring, 0), Data(kSummer, pi * 0.5),
                        Data(kAutumn, pi), Data(kWinter, pi * 1.5)};
  ...
  const double& operator[](Season season) const {
    ...
    if (p == pmap.end())
      throw Error(
          "astrolabe::equinox::SeasonToCircle::const double &operator[]: "
          "season out of range = " +
          int_to_string(season));
    return p->second;
  };
...
double astrolabe::equinox::equinox_exact(double jd, Season season,
                                         double delta) {
  ...
  Parameters:
      ...
      season : one of ("spring", "summer", "autumn", "winter")
      model  : the solar position model, which must be an object that responds
  that to the dimension3() method. Actually, we presume this is an instance of
  astrolabe.sun.Sun() because we use the VSOP coordinate correction function.
  ...
  */
  ...
  for (int i = 0; i < 20; i++) {
    ...
    if (fabs(jd - jd0) < delta) return jd;
  }
