...
/*
   ...
   longitude of the Sun is simply the heliocentric longitude of the Earth + 180
   degrees. The geocentric latitude of the Sun is the negative of the
   heliocentric latitude of the Earth. The radius is of course the same in both
   coordinate systems.
...
void astrolabe::sun::Sun::dimension3(double jd, double& L, double& B,
                                     double& R) const {
...
void astrolabe::sun::longitude_radius_low(double jd, double& L, double& R) {
  ...
  */
  ...
  // Constant terms
  //
  static const double _kL0[] = {d_to_r(280.46646), d_to_r(36000.76983),
                                d_to_r(0.0003032)};
  static const VECTOR(kL0, double);

  static const double _kM[] = {d_to_r(357.52911), d_to_r(35999.05029),
                               d_to_r(-0.0001537)};
  static const VECTOR(kM, double);

  static const double _kC[] = {d_to_r(1.914602), d_to_r(-0.004817),
                               d_to_r(-0.000014)};
  ...
  const double C = polynomial(kC, T) * sin(M) + (_ck3 - _ck4 * T) * sin(2 * M) +
                   _ck5 * sin(3 * M);
  L = modpi2(L0 + C);
  const double v = M + C;
