...
namespace {
const TableA _tblLR[] = {{0, 0, 1, 0, 6288774, -20905355},
...
static void _constants(double T, double& L1, double& D, double& M, double& M1,
                       double& F, double& A1, double& A2, double& A3, double& E,
                       double& E2) {
  // Calculate values required by several other functions"""
  //
  // Constant terms.
  //
  static const double _kL1[] = {d_to_r(218.3164477), d_to_r(481267.88123421),
                                d_to_r(-0.0015786), d_to_r(1.0 / 538841),
                                d_to_r(-1.0 / 65194000)};
  static const VECTOR(kL1, double);

  static const double _kD[] = {d_to_r(297.8501921), d_to_r(445267.1114034),
                               d_to_r(-0.0018819), d_to_r(1.0 / 545868),
                               d_to_r(-1.0 / 113065000)};
  static const VECTOR(kD, double);

  static const double _kM[] = {d_to_r(357.5291092), d_to_r(35999.0502909),
                               d_to_r(-0.0001536), d_to_r(1.0 / 24490000)};
  static const VECTOR(kM, double);

  static const double _kM1[] = {d_to_r(134.9633964), d_to_r(477198.8675055),
                                d_to_r(0.0087414), d_to_r(1.0 / 69699),
                                d_to_r(-1.0 / 14712000)};
  static const VECTOR(kM1, double);

  static const double _kF[] = {d_to_r(93.2720950), d_to_r(483202.0175233),
                               d_to_r(-0.0036539), d_to_r(-1.0 / 3526000),
...
void astrolabe::elp2000::ELP2000::dimension3(double jd, double& longitude,
                                             double& latitude,
                                             double& radius) const {
  ...
  */
  ...
  double lsum = 0.0;
  double rsum = 0.0;
  for (std::vector<TableA>::const_iterator p = tblLR.begin(); p != tblLR.end();
       ++p) {
    double tl = p->l;
    double tr = p->r;
  ...
  //
  double bsum = 0.0;
  for (std::vector<TableB>::const_iterator q = tblB.begin(); q != tblB.end();
       ++q) {
    double tb = q->b;
  ...
  bsum += -2235 * sin(L1) + 382 * sin(A3) + 175 * sin(A1 - F) +
...
double astrolabe::elp2000::ELP2000::dimension(double jd, Coords dim) const {
  ...
  */
  if (dim == vL) return longitude(jd);
  if (dim == vB) return latitude(jd);
  if (dim == vR) return radius(jd);
  throw Error("astrolabe::elp2000::ELP2000::dimension: unknown dimension = " +
              int_to_string(dim));
}
...
double astrolabe::elp2000::ELP2000::latitude(double jd) const {
  ...
  */
  ...
  double bsum = 0.0;
  for (std::vector<TableB>::const_iterator p = tblB.begin(); p != tblB.end();
       ++p) {
    double tb = p->b;
    const double arg = p->D * D + p->M * M + p->M1 * M1 + p->F * F;
    if (fabs((double)p->M) == 1)
