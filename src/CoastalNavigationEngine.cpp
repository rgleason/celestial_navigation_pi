...
RangeResult SolveVerticalAngle(const VerticalAngleObservation& observation) {
  ...
  if (observation.mode == VerticalAngleMode::SeaHorizonToTopBeyondHorizon) {
    ...
    const double height_difference_ft =
        (result.effective_height_m - observation.eye_height_m) / 0.3048;
    ...
    for (int iteration = 0; iteration < 100; ++iteration) {
      ...
      const double angle = VerticalSubtendedAngle(middle, effective_radius,
    ...
    if (result.range_nm > 0.0 &&
      ...
      result.warnings.push_back(
          "Curvature, eye height and standard terrestrial refraction change "
          "the result by more than 3% from the simple height/tangent estimate");
  }
...
GeoPoint Destination(const GeoPoint& start, double bearing_deg,
  ...
  const double lat2 = std::asin(
      Clamp(std::sin(lat1) * std::cos(angular) +
...
HorizontalFixResult SolveHorizontalThreePointFix(
  ...
  if (!(left_centre_angle_deg > 0.0 && left_centre_angle_deg < 180.0 &&
        centre_right_angle_deg > 0.0 && centre_right_angle_deg < 180.0)) {
    result.error =
        "Horizontal sextant angles must be between 0 and 180 degrees";
    return result;
  }
  if (!std::isfinite(observation.first_time_offset_seconds) ||
      ...
      !std::isfinite(observation.speed_knots) ||
      observation.speed_knots < 0.0) {
  ...
  for (int iteration = 0; iteration < 50; ++iteration) {
    ...
    const double scale =
        std::max(1.0, std::max(std::fabs(dlat), std::fabs(dlon)) / 0.25);
    dlat /= scale;
    dlon /= scale;
  ...
  if (std::hypot(result.first_residual_arcmin, result.second_residual_arcmin) >
      0.01) {
    result.error =
        "Horizontal-angle solution did not converge from the supplied "
  ...
  const double frobenius =
      std::sqrt(j00 * j00 + j01 * j01 + j10 * j10 + j11 * j11);
  result.geometry_condition =
      std::fabs(determinant) > 0.0
          ? frobenius * frobenius / std::fabs(determinant)
...
std::vector<std::vector<GeoPoint>> BuildHorizontalAngleLocus(
  ...
  const GeoPoint midpoint =
      Destination(first, midpoint_bearing, baseline_nm / 2.0);
  ...
  for (double side : {-1.0, 1.0}) {
    ...
    for (int index = 0; index <= samples_per_circle; ++index) {
      const GeoPoint point =
          Destination(centre, index * 360.0 / samples_per_circle, radius_nm);
