// Production Sun-Moon adapter for v2.8.5.2; separate from frozen lab drivers.
// Deliberately no analytical fallback: a missing kernel is an error.
...
class Ephemeris {
 public:
  ...
  lunar_distance::EphemerisSample sample(double offset, bool report = false) {
    ...
    const auto dut1 = eclipse::LookupDut1(jd);
    double tai = std::isfinite(dut1.tai_minus_utc) ? dut1.tai_minus_utc : eclipse::TaiMinusUtcSeconds(date);
    ...
    orientation.tt_jd = tt;
    orientation.ut1_jd = jd + dut1.seconds/86400;
    s.dut1_available = dut1.available;
    s.dut1_from_update = dut1.from_update;
    s.dut1_seconds = dut1.seconds;
    s.dut1_quality = dut1.quality;
    s.observer_direction = [this,et,orientation](double lat,double lon,double height,
        bool moon,double* alt,double* az,double* sd) {
      std::string error;
      return eclipse::ObserverApparentDirection(kernel_,et,orientation,lat,lon,height,
                                                moon,alt,az,sd,&error);
    };
