...
Vector3 EarthFixedToIcrf(const Vector3& fixed,
...
Vector3 GeodeticEarthFixed(const GeoPoint& point, double height_metres,
  ...
  const double normal = equatorial_radius_km /
                        std::sqrt(1.0 - e2 * sin_latitude * sin_latitude);
...
bool RayEllipsoidIntersection(const Vector3& origin, const Vector3& direction,
                              ...
                              Vector3* point) {
  const double polar_radius_km =
      equatorial_radius_km * ellipsoid.polar_ratio();
...
double CircleOverlap(double r1, double r2, double distance) {
  ...
  const double c1 = std::max(-1.0, std::min(1.0,
      (distance * distance + r1 * r1 - r2 * r2) / (2.0 * distance * r1)));
  const double c2 = std::max(-1.0, std::min(1.0,
      (distance * distance + r2 * r2 - r1 * r1) / (2.0 * distance * r2)));
  const double term = std::max(0.0, (-distance + r1 + r2) *
...
ShadowFootprint CentralShadowFootprint(
  ...
  result.central = ShadowAxisPosition(state, orientation, ellipsoid, &result.axis);
  ...
  const double apex_distance = constants.moon_radius_km * separation /
                               (constants.sun_radius_km -
                                constants.moon_radius_km);
  ...
  // Earth is before the umbral apex for a total eclipse and beyond it for an
  // annular eclipse. The same double cone handles both cases.
  const double axis_parameter = Dot(Vector3() - apex, away_from_sun);
  result.total = axis_parameter < 0.0;
  const Vector3 cone_axis = result.total ? away_from_sun * -1.0 : away_from_sun;

  Vector3 helper = std::fabs(cone_axis.z) < 0.9
                       ? Vector3(0.0, 0.0, 1.0)
                       : Vector3(0.0, 1.0, 0.0);
  ...
  for (int sample = 0; sample < angular_samples; ++sample) {
    ...
    const Vector3 radial = basis_u * std::cos(theta) +
                           basis_v * std::sin(theta);
...
bool SelectCrossTrackLimits(const ShadowFootprint& footprint,
  ...
  for (std::size_t index = 0; index < footprint.boundary.size(); ++index) {
    ...
    const double east = NormalizeLongitude(
        point.longitude_deg - footprint.axis.longitude_deg) * std::cos(latitude);
...
double SurfaceDistanceKm(const GeoPoint& first, const GeoPoint& second) {
  ...
  const double dlon = DegreesToRadians(NormalizeLongitude(
      second.longitude_deg - first.longitude_deg));
  ...
  const double haversine = sine_lat * sine_lat + std::cos(lat1) *
      std::cos(lat2) * sine_lon * sine_lon;
  return 2.0 * 6371.0088 * std::asin(std::sqrt(
      std::max(0.0, std::min(1.0, haversine))));
}
...
LocalCircumstances EvaluateLocalCircumstances(
  ...
  const Vector3 observer_icrf = EarthFixedToIcrf(observer_fixed, orientation);
  const Vector3 sun_vector = state.sun_from_earth_km - observer_icrf;
  const Vector3 moon_vector = state.moon_from_earth_km - observer_icrf;
  ...
  const double cosine = std::max(-1.0, std::min(1.0,
      Dot(sun_vector, moon_vector) / (sun_distance * moon_distance)));
  ...
  result.total = result.partial &&
                 ...
                 result.separation_rad <= result.moon_semidiameter_rad -
                                              result.sun_semidiameter_rad;
  ...
  if (result.total || result.annular) {
    result.magnitude = result.moon_semidiameter_rad /
                       result.sun_semidiameter_rad;
  } else {
    result.magnitude = std::max(0.0,
        (result.sun_semidiameter_rad + result.moon_semidiameter_rad -
         result.separation_rad) / (2.0 * result.sun_semidiameter_rad));
  }
  const double overlap = CircleOverlap(result.sun_semidiameter_rad,
                                       result.moon_semidiameter_rad,
                                       result.separation_rad);
  result.obscuration = overlap /
      (3.14159265358979323846 * result.sun_semidiameter_rad *
       result.sun_semidiameter_rad);

  const Vector3 sun_fixed = IcrfToEarthFixed(Normalize(sun_vector), orientation);
  ...
  result.sun_altitude_deg = RadiansToDegrees(std::asin(
      std::max(-1.0, std::min(1.0, Dot(sun_fixed, up)))));
  result.sun_azimuth_deg = RadiansToDegrees(
      std::atan2(east_component, north_component));
