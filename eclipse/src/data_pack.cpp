 ...
 private:
  void Transform(const unsigned char block[64]) {
    ...
    for (int index = 0; index < 64; ++index) {
      const std::uint32_t upper = RotateRight(e, 6) ^ RotateRight(e, 11) ^
                                  RotateRight(e, 25);
      const std::uint32_t choose = (e & f) ^ ((~e) & g);
      const std::uint32_t t1 = h + upper + choose + k[index] + words[index];
      const std::uint32_t lower = RotateRight(a, 2) ^ RotateRight(a, 13) ^
                                  RotateRight(a, 22);
...
DataPackStatus VerifyDe440s(const std::string& path) {
  ...
  if (!input) {
    status.error = "Unable to open DE440s data file: " + path;
    return status;
  }
  Sha256 digest;
  ...
  if (!input.eof()) {
    status.error = "Error while reading DE440s data file";
    return status;
  }
  ...
  if (status.sha256 != ExpectedDe440sSha256()) {
    status.error = "DE440s SHA-256 mismatch; the file is corrupt or is not "
