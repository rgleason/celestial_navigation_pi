...
bool CalendarToJulianDate(const CalendarDateTime& calendar, double* jd,
  ...
  double first = 0.0;
  double second = 0.0;
  if (eraCal2jd(calendar.year, calendar.month, calendar.day,
                &first, &second) != 0) {
    if (error) *error = "Invalid Gregorian calendar date";
    return false;
  }
  const double seconds = calendar.hour * 3600.0 + calendar.minute * 60.0 +
                         calendar.second;
...
CalendarDateTime JulianDateToCalendar(double jd) {
  ...
  double fraction = 0.0;
  if (eraJd2cal(2451545.0, jd - 2451545.0, &result.year,
                &result.month, &result.day, &fraction) == 0) {
...
double DecimalYear(const CalendarDateTime& calendar) {
  ...
  const double fraction = (calendar.hour +
      (calendar.minute + calendar.second / 60.0) / 60.0) / 24.0;
...
double ModelDeltaTSeconds(double year) {
  ...
  if (year < 1961.0) {
    t = year - 1950.0;
    return 29.07 + 0.407 * t - t * t / 233.0 +
           t * t * t / 2547.0;
  }
  if (year < 1986.0) {
    t = year - 1975.0;
  ...
  if (year < 2005.0) {
    ...
    return 63.86 + 0.3345 * t - 0.060374 * t * t +
           0.0017275 * t * t * t + 0.000651814 * std::pow(t, 4) +
...
double TdbMinusTtSeconds(double tt_jd, double ut1_jd) {
  ...
  return eraDtdb(2451545.0, tt_jd - 2451545.0, fractional_ut1,
                 0.0, 0.0, 0.0);
}
