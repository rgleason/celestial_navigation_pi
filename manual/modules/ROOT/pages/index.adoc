...
position when other means are not available. The plugin also supports
cross-checking positions obtained by other methods and visualizing a
celestial fix. The source, installation guide and discussion forum are linked
below.
...
Four observation types and separate coastal sextant tools are supported:
...
Measure the angle between the Moon and another heavenly body together with
both altitudes. The plugin clears the distance, searches for matching UTC,
recovers a constant watch offset and reports the two spherical position
intersections. See xref:lunar-coastal.adoc[Lunar distance, watch recovery and
coastal sextant methods].

=== Horizon event

Record the first upper-limb appearance at sunrise or final upper-limb
disappearance at sunset, optionally with a corrected magnetic or true bearing.
The event supplies an altitude line of position. An optional bearing can give
two nominal position candidates; neither is a confidence area or a complete
fix. See xref:offline-planning.adoc[the horizon-event guide].

=== Coastal sextant tools

The separate *Coastal Sextant...* window calculates vertical-angle ranges and
three-object horizontal-sextant-angle fixes. These terrestrial methods are
not the same as a celestial Azimuth sight. See
xref:lunar-coastal.adoc[Lunar distance, watch recovery and coastal sextant
methods].
...
Shifted sights are excluded by the stationary numerical algorithms. Use
*Fix… → Propagate every sight to a common epoch → Each sight's DR Shift* to
include them in a numerical running fix. The chart also shows their advanced
circles for visual use.
...
When a DR Shift is entered the Circle of Position moves accordingly. The
stationary Fix calculation excludes shifted sights; choose *Each sight's DR
Shift* under *Time-tagged running fix* to calculate a numerical fix from them.
When visible sights have saved DR Shifts, the Fix dialog selects that running
fix mode on opening and shows the saved distance and bearing for review.
...
minutes apart), so that the fix can be more accurately determined.
...
group.
. The advanced circles can be inspected visually, or the *Fix…* dialog can
calculate their common-epoch intersection using the individually entered
shifts. The latest sight normally has a zero shift.
...
Enter Eye Height, Temperature, Pressure and Index Error. The optional
*Remarks* field records a short note with this sight; it does not affect the
calculation. Remarks also appear in the Sights list. Sunrise and sunset
observations have the same field in their Horizon Event editor.

image::sight-4-parameters.png["Sight 4 Parameters",width=550,link="_images/sight-4-parameters.png"]

=== Backing up and importing sights

The plugin saves the current log automatically in `Sights.xml`. Use *Manage
Sights…* on the Sights window to save a dated copy; the save dialog starts in
Downloads, but you can choose another location. *Import sights (add to current
log)…* adds all sights and saved lunar solutions from another Celestial
Navigation XML file. It requires the same global clock correction so imported
sight times retain their meaning. *Restore sights (replace current log)…*
replaces the log and its clock correction with those from the chosen file.
Both import actions validate the file and save a dated safety backup of the
current log before changing it.

Older `Sights.xml` files without Remarks still open with empty Remarks fields.
Older plugin versions may discard Remarks if they subsequently rewrite a new
file, so keep a backup before going back to an older version.
...
== Offline planner, running fix and sight analysis

This maintained version adds an entirely offline *Sun & Moon…* planner,
time-tagged numerical running fixes, a sight-sequence analyzer, noon/Polaris
helpers and almanac CSV export. See
xref:offline-planning.adoc[Offline planning, running fixes and sight analysis]
for controls, algorithms, limitations and safety notes.

Creating a sight selects it and clears the previous selection. Deleting the
selected sight moves selection to an adjacent sight when one remains; there is
therefore no unselected state while the sight list is non-empty.

The old manual DR Shift and visual-fix process remains available. Alternatively,
*Fix…* can propagate visible observations with one COG/SOG model or their saved
per-sight shifts to a common epoch and solve them numerically without changing
the stored raw sight times.
...
Manual DR Shift still produces graphically advanced circles for visual use.
This maintained version also offers a separate time-tagged running-fix mode in
*Fix…*. It keeps each raw observation at its actual UTC, propagates a candidate
position through a COG/SOG motion model, and solves latitude/longitude at a
chosen common epoch. See xref:offline-planning.adoc[the running-fix guide].
