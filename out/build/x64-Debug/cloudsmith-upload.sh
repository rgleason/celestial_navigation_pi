...
if [ -n "${OCPN_TARGET}" ]; then
    tarball_name=celestial_navigation_pi-2.4.43.2-${PKG_TARGET}-x86-wx32-${PKG_TARGET_VERSION}-${OCPN_TARGET}-tarball
else
    tarball_name=celestial_navigation_pi-2.4.43.2-${PKG_TARGET}-x86-wx32-${PKG_TARGET_VERSION}-tarball
fi
...
if [ "$BUILD_BRANCH_LOWER" = "master" ]; then
    ...
    if [ -n "$BUILD_TAG"  ]; then
        ...
        VERSION="2.4.43.2+${BUILD_ID}.${commit}"
...
# Only upload if the owner did the pull request. Non-owners probably don't have security to upload to the cloudsmith repository
    if [ "$CIRCLE_PROJECT_USERNAME" = "$CIRCLE_USERNAME" ]; then
        cloudsmith push raw --republish --no-wait-for-sync \
            --name celestial_navigation_pi-2.4.43.2-msvc-x86-wx32-10.0.26100-${OCPN_TARGET}-metadata \
            --version ${VERSION} \
            --summary "celestial_navigation opencpn plugin metadata for automatic installation" \
            $REPO $xml

        cloudsmith push raw --republish --no-wait-for-sync \
            --name $tarball_name \
            --version ${VERSION} \
            --summary "celestial_navigation opencpn plugin tarball for automatic installation" \
            $REPO $tarball

        if [ "${PKG_EXT}" != "gz" ] && [ "$pkg" != "" ]; then
            cloudsmith push raw --republish --no-wait-for-sync \
                --name opencpn-package-celestial_navigation-2.4.43.2-msvc-x86-wx32-10.0.26100-${OCPN_TARGET}.${PKG_EXT} \
                --version ${VERSION} \
                --summary "celestial_navigation .${PKG_EXT} installation package" \
                $REPO $pkg
        fi
    fi
fi
