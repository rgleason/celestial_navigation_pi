#!/usr/bin/env bash
set -euo pipefail
manual_python="${MANUAL_PYTHON:-python3}"
manual_office="${MANUAL_LIBREOFFICE:-libreoffice}"
...
if [[ "${MANUAL_REGENERATE_DIAGRAMS:-1}" != 0 ]]; then
  "$manual_python" "$script_dir/generate_diagrams.py"
fi
mkdir -p "$output_dir/images"
cp "$script_dir"/images/*.png "$output_dir/images/"
mkdir -p "$plugin_data_dir/images"
"$manual_python" "$script_dir/prepare_plugin_html.py"
...
"$manual_office" "-env:UserInstallation=file://$lo_profile_docx" --headless \
...
"$manual_python" "$script_dir/embed_docx_images.py" \
  "$output_dir/Celestial_Navigation_Manual_v2.docx"

"$manual_office" "-env:UserInstallation=file://$lo_profile_pdf" --headless \
...
"$manual_python" "$script_dir/validate_manual.py"
